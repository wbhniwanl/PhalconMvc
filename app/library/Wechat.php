...
//         if(YII_ENV == 'development' || YII_ENV == 'local')
//             return false;
        if (!$appid || !$appsecret) {
            $appid = $this->appid;
        ...
        //$this->access_token = $this->getLocalAccessToken();
        ...
        if ($result) {
            ...
            if (!$json || isset($json['errcode'])) {
                ...
                if($json['errcode'] ==40001){
                    $this->updateToken($appid);
                }
                Yii::log("access_token失败，原因errCode:{$json['errcode']},errMsg:{$json['errmsg']}",'info','api_weixin_error_assess_token');
                return false;
            }
            $this->access_token = $json['access_token'];
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            //$this->saveLocalAccessToken($this->access_token, $expire);
    ...
    {
        $upTokenTime = Yii::app()->dbyou->createCommand()
            ->update('bage_access_token',array('expires_in'=>0),'alias=:alias',array(':alias'=>$appid));
    }


    /**
     * 将密钥保存至数据库，防止频繁获取密钥，以免影响业务
     * @param string $access_token 密钥
     * @param number $expire 有效期时间（秒数）
     * @return integer
     */
    public function saveLocalAccessToken($access_token, $expire){
    	$appid = $this->appid;
    	$expire_time = $expire ? (time() + $expire) : 0;
    	$command = Yii::app()->dbyou->createCommand();
    	
    	$res = $command->from('bage_access_token')
        	->where('alias=:alias', array(':alias'=>$appid))
        	->queryRow();
    	//不存在插入，存在则更新过期时间
    	if ($res){
    	    $res = $command->update('bage_access_token',array(
    	        'access_token'=>$access_token,'expires_in'=>$expire_time
    	    ),"alias=:alias", array(':alias'=>$appid));
    	}else{
    ...
    public function getLocalAccessToken() {
        $accessData = Yii::app()->dbyou->createCommand()
                ->from('bage_access_token')
                ->select("access_token,expires_in")
                ->where('alias=:alias', array(':alias'=>$this->appid))
    ...
    public function setAccessTokenTimeout(){
        return Yii::app()->dbyou->createCommand()->update('bage_access_token',array(
            'expires_in'=>0
        ),"alias=:alias", array(':alias'=>$this->appid));
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '') {
        ...
        Yii::log("getJsTicket by weixin start",'info','api_ticket_weixin');
        ...
        if ($result) {
            ...
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                Yii::log("errCode=".$this->errCode." errMsg=".$this->errMsg,'info','api_weixin_error');
                return false;
            }
