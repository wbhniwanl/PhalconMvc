...
class XCurl extends Component
{
    const VERSION       = '2.1.0';
    const NET_SLOW_TIME = 3;
    ...
    private $no_decode            = false; //不处理返回的json
    ...
    public function __construct()
    {
        ...
        $this->setOpt(CURLOPT_TIMEOUT, 8);
        $this->setOpt(CURLOPT_DNS_CACHE_TIMEOUT, 3600);
        $this->setHeader('Connection', 'keep-alive');

    }

    public function get($url_mixed, $data = array(), $no_decode = false)
    {

        $this->no_decode = $no_decode;
    ...
    //并发多个请求，并且每个参数可以不同
    public function multiGet($url_mixed, $data = array(), $headers = array(), $debug = false, $over_time = 15)
    {
        if (is_array($url_mixed)) {
            $curl_multi         = curl_multi_init();
            $this->multi_parent = true;
            ...
            foreach ($url_mixed as $url_key => $url) {
                $curl              = new Curl();
                $curl->multi_child = true;
                $curl->setOpt(CURLOPT_URL, $this->buildURL($url, $data[$url_key]), $curl->curl);
                $curl->setOpt(CURLOPT_CUSTOMREQUEST, 'GET');
                $curl->setOpt(CURLOPT_HTTPGET, true);
                if (is_array($headers) && !empty($headers)) {
                    foreach ($headers as $headers_key => $headers_value) {
                        $curl->setHeader($headers_key, $headers_value);
                    }
                }
    ...
    {
        $user_agent = 'PHP-Curl-Class/' . self::VERSION . ' (yaochufa.com)';
        ...
        if (!(strpos($response, "\r\n\r\n") === false)) {
            ...
            if (isset($response_headers['Content-Type'])) {
                if (preg_match('/^application\/json/i', $response_headers['Content-Type'])) {
                    $json_obj = json_decode($response, $this->no_decode);
                    if (!is_null($json_obj)) {
                        $response = $json_obj;
                    }
    ...
    protected function exec($_ch = null)
    {
        ...
        $ch_res                 = curl_getinfo($ch->curl);
        $ch->http_status_code   = $ch_res['http_code'];
        $ch->http_totel_time    = $ch_res['total_time'];

        if ($ch->http_totel_time > self::NET_SLOW_TIME) {
            Yii::log(serialize($ch_res), 'info', 'slow_api_strace_' . $ch->http_totel_time);
        }
